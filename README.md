...
- **`skill/`** — *Out of scope for this repo.* The CoWork skill that picks up Drive submissions and writes Pending rows to the Intranet sheet is built separately.
...
4. *(Out of scope)* CoWork skill processes the folder hourly, writes Pending rows to the IntranetControl Sheet, calls `POST /admin/notify`.
...
7. Intranet renders the new Live row on next refresh.
